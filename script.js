        ...
        if (proceedBtn) {
            proceedBtn.addEventListener('click', () => {
                ...
                const item = cart[0];
                const totalAmount = (item.price * item.quantity).toFixed(2).replace('.', ',');
                const deliveryInfo = shippingData.deliveryType === 'express' 
                    ? 'Entrega Rápida - Em até 20 minutos' 
                    : `Entrega Agendada - ${shippingData.deliveryDate} às ${shippingData.deliveryTime}`;
                ...
                const templateParams = {
                    ...
                    customer_name: shippingData.customerName || 'Cliente',
                    customer_phone: shippingData.customerPhone || '',
                    customer_email: 'cliente@email.com',
                    address: `${shippingData.street}, ${shippingData.number}${shippingData.complement ? ' - ' + shippingData.complement : ''}, ${shippingData.neighborhood} - ${shippingData.city}/${shippingData.state}, CEP: ${shippingData.cep}`
                };

                emailjs.send('service_0euy74r', 'template_57ynkas', templateParams)
                    .then((response) => {
